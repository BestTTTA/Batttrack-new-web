...
const Checkpage = () => {
  ...
  return (
    <section id="check_page" className="overflow-x-auto p-4 whitespace-nowrap h-screen min-w-full bg-gray-700 transition duration-500 ease-in-out">
      {
        data && data.length > 0 ? (
              ...
              <div key={details.id || index} className='flex flex-col bg-gray-200 h-fit rounded-md drop-shadow-lg p-2'>
                ...
                <div className='w-full h-full grid grid-flow-col'>
                  <div className='flex h-full flex-col justify-between'>
                    ...
                    <p className='text-[13px]'>{details.list_serial?.[0]?.process_status === true ? "ดำเนินการเสร็จสิ้น" : "N/A"}</p>
                  </div>
                  <div className='flex h-full flex-col justify-between'>
                  ...
                  <div className='flex h-full flex-col justify-center'>
                    <button onClick={() => { GetdataAndNavigation(details.list_serial?.[0]?.serial_number) }} className='underline focus:text-orange-500 focus:scale-105 text-sm'>ตรวจสอบสถานะ</button>
                  </div>
                </div>
              </div>
            ))}
